...
class PlanPricingController extends Controller
{
    ...
    public function select_plan($id)
    {
        ...
        // if ($data['original']['status'] == 2 && $data['original']['banktransfer'] == 1) {
        //     return redirect('admin/plan')->with('error', $data['original']['message']);
        // }
        if ($plan->price > 0) {
            $paymentmethod = Payment::whereNotIn('payment_name', ['wallet'])->where('vendor_id', '1')->where('is_available', '1')->where('is_activate', 1)->get();
    ...
    public function status_change(Request $request){
        $status = PricingPlan::where('id', $request->id)->update(['is_available'=>$request->status]);
        if ($status) {
            return 1;
        } else {
            return 2;
