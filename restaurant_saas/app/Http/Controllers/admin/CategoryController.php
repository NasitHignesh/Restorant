...
class CategoryController extends Controller
{
    public function index(){
        $getcategory = Category::where('is_deleted','2')->orderBy('reorder_id')->where('vendor_id',Auth::user()->id)->get();
    ...
    public function subcategory_index(Request $request){
        $getsubcategory = Subcategory::with('category_info')->where('is_deleted',2)->orderByDesc('id')->where('vendor_id',Auth::user()->id)->get();
        return view('admin.subcategory.index',compact('getsubcategory'));
    }
    public function subcategory_add(Request $request){
        $getcategory = Category::where('is_available',1)->where('is_deleted',2)->orderByDesc('id')->where('vendor_id',Auth::user()->id)->get();
