@if (count($getitem) > 0)
    <div class="row item-list-view">
        @foreach ($getitem as $item)
            <div class="col-lg-3 col-md-4 col-sm-6 mt-5 mb-2">
                <div class="card border-0 card-section text-center">
                    @php
                    ...
                    <div class="d-flex justify-content-center">
                        @if ($item->addons_id != '' || count($item->variation) > 0)
                            <a class="btn px-2 text-dark"
                                onclick="showitem('{{ $item->id }}','{{ URL::to('admin/pos/show-item') }}')"><i
                                    class="fa fa-shopping-bag px-1"></i>{{ trans('labels.add_to_cart') }}</a>
                        @else
                            <a class="btn px-2 text-dark"
                                onclick="addcart('{{ $item->id }}','{{ $item->item_name }}','{{ $item->item_type }}','{{ $image_name }}','{{ $item->tax }}','{{ $item->price }}','{{ URL::to('admin/pos/addtocart') }}')"><i
                                    class="fa fa-shopping-bag px-1"></i>{{ trans('labels.add_to_cart') }}</a>
                        @endif
                    </div>
                </div>
