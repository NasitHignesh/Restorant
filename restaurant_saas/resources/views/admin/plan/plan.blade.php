...
<div class="row justify-content-between align-items-center mb-3">
    <h5 class="text-uppercase">{{ trans('labels.subscription plan') }}</h5>
    <div class="d-flex justify-content-end align-items-center">
        <a href="{{ URL::to('admin/plan/add') }}" class="btn btn-secondary px-2 d-flex">
...
<div class="row mb-7">
    ...
    <div class="col-xl-3 col-lg-6 col-md-6 col-sm-6 mt-3">
        <div class="card box-shadow border-0 h-100 text-dark plancard">
            <div class="card-body">
                <div class="mb-4">
                    <h1 class="mb-4 plan-price">{{ Helper::currency_formate($plandata->price, '1') }}
            ...
            <div class="card-footer bg-white border-top-0 my-2 text-center d-flex justify-content-between align-items-center">
                @if (Auth::user()->type == '1')
                <div>
                    <a href="{{ URL::to('admin/plan/edit-' . $plandata->id) }}" class="btn btn-sm btn-info btn-size" tooltip="Edit">
                        <i class="fa-regular fa-pen-to-square "></i> </a>
                    @if (env('Environment') == 'sendbox')
                    <a onclick="myFunction()"> <i class="fa-regular fa-trash"></i></a>
                    @else
                    <a onclick="changestatus('{{$plandata->id}}','','{{ URL::to('admin/plan/delete') }}')" tooltip="Delete" class="btn btn-sm btn-danger btn-size">
                ...
                <a onclick="myFunction()" class="btn btn-success  btn-sm w-50 mt-2 rounded-5">{{ trans('labels.active') }}</a>
                @elseif ($plandata->is_available == 2)
                <a onclick="myFunction()" class="btn btn-danger w-50 btn-sm mt-2 rounded-5">{{ trans('labels.inactive') }}</a>
                @endif
                @else
                @if ($plandata->is_available == 1)
                <a onclick="changestatus('{{$plandata->id}}','2','{{ URL::to('admin/plan/status_change') }}')" class="btn btn-success  btn-sm w-50 mt-2 rounded-5">{{ trans('labels.active') }}</a>
                @elseif ($plandata->is_available == 2)
                <a onclick="changestatus('{{$plandata->id}}','1','{{ URL::to('admin/plan/status_change') }}')" class="btn btn-danger w-50 btn-sm mt-2 rounded-5">{{ trans('labels.inactive') }}</a>
                @endif
                @endif
                @else
                ...
                <small class="text-success d-block fw-500 text-start"><span>{{ @$data['original']['plan_message'] }}</span></small>
                @else
                @if (@$data['original']['plan_date'] > date('Y-m-d'))
                <small class="text-dark d-block fw-500 text-start">{{ @$data['original']['plan_message'] }}
                    : <span class="text-success">{{$data['original']['plan_date'] !="" ? Helper::date_format($data['original']['plan_date']):'' }}</span>
                </small>

                @else
                <small class="text-dark d-block fw-500 text-start">{{ @$data['original']['plan_message'] }}
                    : <span class="text-danger">{{$data['original']['plan_date'] !="" ? Helper::date_format($data['original']['plan_date']) : ''}}</span>
                </small>
                ...
                <a href="{{ URL::to('admin/plan/selectplan-' . $plandata->id) }}" class="btn btn-sm d-block mt-2 bg-success text-white rounded-5 px-3">{{ trans('labels.subscribe') }}</a>
                @endif
                @else
                @if (@$data['original']['plan_date'] > date('Y-m-d'))
                <small class="text-dark d-block fw-500 text-start">{{ @$data['original']['plan_message'] }}
                ...
                <a href="{{ URL::to('admin/plan/selectplan-' . $plandata->id) }}" class="btn btn-sm d-block bg-success text-white rounded-5 px-3">{{ trans('labels.subscribe') }}</a>
                @else
                <small class="text-dark d-block fw-500 text-start">{{ @$data['original']['plan_message'] }}
                ...
                <small class="text-dark fw-500 text-start">
                    {{ @$data['original']['plan_message'] }}: <span class="text-success">{{ $data['original']['plan_date'] !="" ? Helper::date_format($data['original']['plan_date']) : ''}}</span>
                </small>
                <div class="d-flex justify-content-end w-100">
                    <a href="{{ URL::to('admin/plan/selectplan-' . $plandata->id) }}" class="btn btn-sm bg-success text-white rounded-5 px-3 d-block">
                        {{ trans('labels.subscribe') }}
                    </a>
                </div>
                @else
                <small class="text-success">{{ @$data['original']['plan_message'] }}</small>
                <a href="{{ URL::to('admin/plan/selectplan-' . $plandata->id) }}" class="btn btn-sm bg-success text-white rounded-5 px-3 d-block">{{ trans('labels.subscribe') }}</a>
                @endif
                @endif
                @else
                -
